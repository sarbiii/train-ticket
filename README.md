...
- `.env` 파일을 사용하지 않습니다.
- 로그인 정보는 웹 화면에서 입력하며, 개인용 편의를 위해 브라우저 localStorage에 저장합니다.
- `저장 로그인 삭제`를 누르면 브라우저 저장값과 실행 중인 서버 메모리 로그인 정보를 삭제합니다.
- 앱 실행 중 `scripts/clear_personal_data.sh`를 실행하면 서버 메모리 로그인 정보를 지우고 브라우저 삭제 페이지를 엽니다.
...
`main` 브랜치 또는 `v*` 태그를 푸시하면 GitHub Actions가 다음 산출물을 만듭니다.
...
검증 후 `v0.1.0` 같은 태그를 푸시하고, GitHub Actions artifact를 Release asset으로 첨부합니다.
...
4. 출발역, 도착역, 날짜, 시간 범위, 성인 인원수를 입력하고 열차를 조회합니다.
5. 스나이핑할 열차를 선택합니다.
6. 좌석 선호, 예약대기 여부, 조회 간격을 선택합니다.
7. `스나이핑 시작`을 누릅니다.
8. 예약 성공 시 브라우저 알림과 화면의 예약 정보를 확인합니다.
9. 디버깅이나 사용을 마친 뒤 `저장 로그인 삭제`를 누르거나 `scripts/clear_personal_data.sh`를 실행합니다.
